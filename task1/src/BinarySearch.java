...
public class BinarySearch {
    ...
    public static void main(String[] args) {
        ...
        // a[i - 1] >= a[i] ^ 0 <= i < a.length ^ a[-1] == +inf
        System.out.println(loopSearch(a, x));
    }

    // Pre: a[i - 1] >= a[i] ^ 0 <= i < a.length ^ a[-1] == +inf
   ...
   // Pre: -1 <= i < a.length ^ a[i - 1] >= a[i] ^ a[-1] == +inf
   // Post: R = r ^ a[r-1] > x >= a[r]
    private static int recursiveSpan(int[] a, int x) {
        return recursiveSearch(a, x, -1, a.length - 1);
    }


    // Pre: -1 <= l < i <= r < a.length ^ a[i - 1] >= a[i] ^ a[-1] == +inf
    // Post: R = r ^ a[r-1] > x >= a[r]
